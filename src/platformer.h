 ...
 *   increased screen size to 1080p, added some platforms and double jump ability. also showing camera
 *   target and offset values so that its easier to understand how the camera works.
 *
 *   *******************************************************************************************/
...
void UpdateCameraCenterInsideMap(Camera2D *camera, Player *player, EnvItem *envItems, int envItemsLength, float delta, int width, int height)
{
    ...
    if (max.x < width)
        camera->offset.x = width - (max.x - width / 2) - PLAYER_WIDTH;
    if (max.y < height)
        camera->offset.y = height - (max.y - height / 2) - PLAYER_HEIGHT;
    if (min.x > 0)
        camera->offset.x = width / 2 - min.x + PLAYER_WIDTH;
    if (min.y > 0)
        camera->offset.y = height / 2 - min.y + PLAYER_HEIGHT;
}
